...
import MPHiElixir.Util.*;
import MPHiElixir.Comms.*;
import MPHiElixir.Debug.*;

import MPHiElixir.fast.*;

public class Destabilizer extends Robot {
    static enum DestabilizerState {
        EXPLORING,
        ATTACKING,
        RUNNING,
        REPORTING,
    ...
    static DestabilizerState currState;
    ...
    public static final int LOW_HEALTH_REPORT_THRESHOLD = 60;
    public static final int LOW_HEALTH_THRESHOLD = 50;
    public static final int LOW_HEALTH_DIFF = 20;
    public static final int HIGH_HEALTH_DIFF = 40;

    public static final int LOW_HEAL_THRESHOLD = 100;
    public static final int HIGH_HEAL_THRESHOLD = 150;

    public static final int MAX_SYM_MANA_SECTORS = 2;

    public static final int ID = 11785;

    public Destabilizer(RobotController r) throws GameActionException {
        super(r);
        currState = DestabilizerState.EXPLORING;
    ...
    public boolean shouldWait() {
        ...
        return !rc.isActionReady() && rc.getLocation().isWithinDistanceSquared(lastClosestAttackingEnemy, 34);
    }
        ...
        switch (currState) {
            case REPORTING:
                ...
                if (closestEnemy != null) {
                    if (shouldRunAway()) {
                        currState = DestabilizerState.RUNNING;
                    } else if (shouldHelpInjured()) {
                        currState = DestabilizerState.HELPING;
                    } else {
                        if (closestEnemy.getType() == RobotType.HEADQUARTERS) {
                            currState = DestabilizerState.EXPLORING;
                            HQwaitCounter = 8;
                        }
                        currState = DestabilizerState.ATTACKING;
                    }
                } else if (hasReported) {
                    if (shouldHeal()) {
                        currState = DestabilizerState.HEALING;
            ...
            default:
                if (shouldHeal()) {
                    ...
                    currState = DestabilizerState.WAITING;
                    ...
                    currState = DestabilizerState.REPORTING;
                    sectorToReport = 1;
                } else {
                    currState = DestabilizerState.EXPLORING;
                }
    ...
    public void moveAndDestabilize(Direction dir) throws GameActionException {
    ...
    public void destabilizeAndMove(Direction dir) throws GameActionException {
        // Sync launcher movement
        tryDestabilize();
        if (rc.canMove(dir)) {
            rc.move(dir);
    ...
    public void moveAndDestabilize(MapLocation loc) throws GameActionException {
        Nav.move(loc);
        tryDestabilize();
    }

    public void destabilizeAndMove(MapLocation loc) throws GameActionException {
        tryDestabilize();
        Nav.move(loc);
    }
    ...
    public boolean shouldHeal() throws GameActionException {
        ...
        for (int i = FriendlySensable.length; --i >= 0;) {
            ...
            // Consider other destabilizers? Don't think it really matters
    ...
    public void destabilizerExplore() throws GameActionException {
    ...
    public void attackClouds() throws GameActionException {
        ...
        if (attackLoc != null && rc.canDestabilize(attackLoc)) {
            rc.destabilize(attackLoc);
        }
    }
    ...
    public void goToNearestIsland() throws GameActionException {
        ...
        if (bestHealLoc == null) {
            ...
            moveAndDestabilize(closestFriendlyIsland);
            return;
        }
        ...
        moveAndDestabilize(bestHealLoc);
    }
    ...
    public boolean tryDestabilize() throws GameActionException {
        if (!rc.isActionReady())
            return false;

        MapLocation enemy = closestEnemyLocation;
        if (enemy == null) {
            if (lastClosestAttackingEnemy == null
                    || turnSawLastClosestAttackingEnemy + LAST_ATTACKING_ENEMY_TIMEOUT < rc.getRoundNum()) {
                // No enemies nearby, so don't destabilize
                return false;
            }
            // Use the lastClosestAttackingEnemy if it was recent enough.
            enemy = lastClosestAttackingEnemy;
        }

        MapLocation destabilizeTarget = Util.getDestabilizeLocToward(enemy);
        if (rc.canDestabilize(destabilizeTarget)) {
            rc.destabilize(destabilizeTarget);
            return true;
        }
